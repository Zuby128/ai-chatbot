...
const Index = () => {
  ...
  const postMessage = async () => {
    try {
      if (text.trim()) {
        const body = { sessionId, text, sender: "user" };
  ...
  return (
    <div className="container mx-auto h-[calc(100vh_-_4rem)] flex justify-center items-center pb-4 md:pb-8">
      <div className="chat-box w-full max-w-xl rounded-lg  h-full flex flex-col-reverse bg-white text-gray-900 relative px-4 pb-[13vh] overflow-auto">
        {messages.length === 0 && (
        ...
        <div className="chat-footer w-full max-w-xl h-[13vh] fixed left-1/2 -translate-x-1/2 bottom-4 md:bottom-8 rounded-t-0 rounded-b-lg bg-gray-200 p-2">
          <div className="w-full h-full flex flex-col md:grid md:grid-cols-5 items-center">
            ...
            <button
              onClick={postMessage}
              className="h-8 min-h-8 md:h-full max-h-12 w-full md:col-span-1 rounded-md flex justify-center items-center bg-teal-600 text-white shadow-lg hover:bg-teal-900"
            >
              <Send color="#fff" />
            </button>
          </div>
